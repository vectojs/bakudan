...
function toNode(entity) {
  ...
  const node = {
    ...
    x: round22(entity.x),
    y: round22(entity.y),
    width: round22(entity.width),
    height: round22(entity.height),
    worldBounds: {
      x: round22(wb.x),
      y: round22(wb.y),
      width: round22(wb.width),
      height: round22(wb.height)
    }
  };
  if (entity.opacity !== 1) node.opacity = round22(entity.opacity);
...
function captureSnapshot(scene) {
  return {
    width: round22(scene.width),
    height: round22(scene.height),
...
function roundBounds2(b) {
  return { x: round23(b.x), y: round23(b.y), width: round23(b.width), height: round23(b.height) };
}
...
function auditTree(root, sceneBounds, opts = {}) {
  ...
  const overflowFinding = (kind, entity, bounds, container, over) => {
    const sides = ["left", "right", "top", "bottom"].filter((s) => over[s] > tolerance).map((s) => `${s} ${round23(over[s])}px`).join(", ");
    findings.push({
      kind,
      entityId: entity.id,
      ...
      overflow: {
        left: round23(over.left),
        right: round23(over.right),
        top: round23(over.top),
        bottom: round23(over.bottom)
      }
    });
  };
      ...
      for (let j = i + 1; j < kids.length; j++) {
        ...
        if (inter && inter.width > tolerance && inter.height > tolerance) {
          findings.push({
            ...
            message: `${a.constructor.name} overlaps sibling ${b.constructor.name} by ${round23(inter.width)}\xD7${round23(inter.height)}px`
          });
        }
      }
    }
