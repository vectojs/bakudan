...
__export(index_exports, {
  ...
  createEventTrace: () => createEventTrace,
  describeEntity: () => describeEntity,
  diffSnapshots: () => diffSnapshots,
  entityPath: () => entityPath,
  findEntityAt: () => findEntityAt,
  inspectEntity: () => inspectEntity,
  pickInScene: () => pickInScene,
  textPreviewOf: () => textPreviewOf
});
...
var DevtoolsPanel = class {
  ...
  constructor(host, options = {}) {
    ...
    this.panelScene = new import_core.Scene(canvas, {
      disableWindowResize: true,
      contentProjection: false
    });
    this.panelScene.renderMode = "onDemand";
    const title = new import_ui.Text("VectoJS DevTools", { font: "bold 14px monospace" });
    title.setPosition(12, 10);
    this.panelScene.add(title);
    const pickBtn = new import_ui.Button("Pick", { width: 64, height: 24 });
    pickBtn.setPosition(12, 34);
    pickBtn.on("click", () => this.armPick());
    this.panelScene.add(pickBtn);
    const refreshBtn = new import_ui.Button("Refresh", { width: 84, height: 24 });
    refreshBtn.setPosition(84, 34);
    refreshBtn.on("click", () => this.refresh());
    this.panelScene.add(refreshBtn);
    const auditBtn = new import_ui.Button("Audit", { width: 64, height: 24 });
    ...
    this.tree = new import_ui.TreeView({
      nodes: [],
      width: this.width - 16,
      height: treeHeight,
      rowHeight: 20,
    ...
    for (let i = 0; i < 8; i++) {
      const line = new import_ui.Text("", { font: "11px monospace" });
...
var HighlightEntity = class extends import_core.Entity {
  target = null;
  track(target) {
    this.target = target;
  }
